...
export default function TreemapChart(g: d3.Selection<SVGGElement, unknown, null, undefined>, props: ChartPluginProps) {
  ...
  leaf.append('rect').attr('width', d => d.x1 - d.x0).attr('height', d => d.y1 - d.y0).attr('fill', (d, i) => colorScale(String(i))).attr('rx', 4).on('mouseover', (e, d) => showTooltip(e, String((d.data as any)[xAxis]), (d.data as any)[yAxis[0]], undefined, d.data)).on('mousemove', moveTooltip).on('mouseout', hideTooltip);
