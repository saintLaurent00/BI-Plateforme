...
export const HierarchicalChartPlugin: ChartPlugin = {
  ...
  render: (g, { data, xAxis, yAxis, width, height, colorScale, showTooltip, moveTooltip, hideTooltip, type }) => {
    ...
    if (type === 'Treemap') {
      ...
      const treeLayout = type === 'Tree' ? d3.tree<any>() : d3.cluster<any>();
      treeLayout.size([height, width - 160])(root);
      ...
      const treeLayout = d3.tree<any>().size([2 * Math.PI, radius]);
      treeLayout(root);

      const radialG = g.append('g')
        .attr('transform', `translate(${width / 2},${height / 2})`);
