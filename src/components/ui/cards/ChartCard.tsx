    ...
    return (
      ...
      >
        <div 
          ...
          className="w-14 h-14 rounded-2xl bg-muted flex items-center justify-center shrink-0 border border-border group-hover:bg-accent group-hover:text-accent-foreground group-hover:border-accent/20 transition-all relative"
        >
          <Icon className="w-5 h-5" />
          
          {/* Mini Preview on list hover */}
          {isHovered && (
            <motion.div 
              initial={{ opacity: 0, scale: 0.8, y: 10 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              className="absolute -top-32 left-0 w-48 h-28 bg-background/95 backdrop-blur-xl border border-accent/20 rounded-2xl shadow-2xl z-50 p-3 pointer-events-none"
            >
              <div className="w-full h-full">
                <MiniChart type={type} />
              </div>
              <div className="absolute -bottom-2 left-6 w-4 h-4 bg-background border-r border-b border-accent/20 rotate-45" />
            </motion.div>
          )}
        </div>
