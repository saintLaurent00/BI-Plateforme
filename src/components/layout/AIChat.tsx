...
export const AIChat = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  const [messages, setMessages] = React.useState<Message[]>([
    { role: 'model', text: "Bonjour ! Je suis l'assistant IA de Hifadih BI. Comment puis-je vous aider aujourd'hui ?" }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const response = await ai.models.generateContent({ 
        ...
        config: {
          systemInstruction: "Tu es l'assistant IA de Hifadih BI, une plateforme d'analyse de données d'entreprise. Sois professionnel, serviable et concis. Réponds en français. Tu as accès aux outils de gouvernance et de screening dans le panneau d'administration pour surveiller la santé de la plateforme et la sécurité des données.",
        }
      });
        ...
        >
          ...
          <div className="p-4 bg-prism-600 text-white flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm">Hifadih AI</h3>
