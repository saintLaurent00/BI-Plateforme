...
export const isValidChild = (type: string, parentType: string): boolean => {
  const rules: Record<string, string[]> = {
    'root': ['grid'],
    'grid': ['row', 'header', 'markdown', 'divider', 'tabs', 'chart'],
    'row': ['column', 'chart', 'markdown', 'header'],
    'column': ['row', 'chart', 'markdown', 'header', 'tabs'],
    'tabs': ['tab'],
    'tab': ['row', 'column', 'chart', 'markdown', 'header']
  };

  return rules[parentType.toLowerCase()]?.includes(type.toLowerCase()) || false;
};
...
export const moveItem = (
  ...
  if (sourceParentId === 'NEW' && newItem) {
    ...
    nextLayout = {
      ...layout,
      [itemId]: { ...newItem, parents: [targetParentId] }
    };
  ...
  // 2. Add to target
  nextLayout = update(nextLayout, {
    [targetParentId]: {
      children: { $splice: [[targetIndex, 0, itemId]] }
    },
    [itemId]: {
      parents: { $set: [targetParentId] }
    }
  });
