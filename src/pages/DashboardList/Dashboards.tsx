              ...
              <DataTable 
                ...
                columns={[
                  { 
                    ...
                    render: (val, row) => (
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-foreground">{val || row.name || row.title}</p>
                          <p className="text-[10px] text-muted-foreground uppercase tracking-widest">{row.id?.toString().substring(0, 8)}</p>
                        </div>
                      </div>
                    )
                  },
                  ...
                  },
                  {
                    ...
                    render: (_, row) => {
                      // Note: In a real app, we'd list the datasets used in the dashboard's charts
                      return <Badge variant="neutral" className="bg-muted text-[10px]">Multiples</Badge>;
                    }
                  },
                  { 
                    key: 'created_on_delta_humanized', 
                    label: 'Créé le',
                    render: (val, row) => val || (row.created_at ? new Date(row.created_at).toLocaleDateString() : 'N/A')
                  },
                  { 
